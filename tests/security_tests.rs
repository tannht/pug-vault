...
mod security_tests {
    ...
    fn test_encrypted_file_content_not_readable() {
        ...
        for part in parts {
            assert!(
                hex::decode(part).is_ok(),
                "Part should be valid hex: {}",
                part
            );
        }
    }
    ...
    fn test_partial_file_corruption() {
        ...
        // Error should be about corrupted file or invalid hex or authentication failure
        let error_msg = result.unwrap_err().to_string();
        assert!(
            error_msg.contains("Corrupted vault file")
                || error_msg.contains("Odd number of digits")
                || error_msg.contains("Invalid character")
                || error_msg.contains("invalid hex")
                || error_msg.contains("Invalid input length")
                || error_msg.contains("Invalid Master Password or corrupted data")
        );
    }
    ...
    fn create_vault_in_dir(temp_dir: &TempDir, password: &str) -> Vault {
        ...
        let data_file = temp_dir
            .path()
            .join(format!("test_vault_{}.data", rand::random::<u32>()));
